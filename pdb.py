...
class Pdb(pdb.Pdb, ConfigurableClass):
    ...
    def _checkline_moving(self, filename, lineno):
        """Check whether specified line seems to be executable.

        Return `lineno` if it is, 0 if not (e.g. a docstring, comment, blank
        line or EOF). Warning: testing is not comprehensive.

        This is mostly copied from the checkline() function in the base pdb.py that accompanied Python 3.4.
        """
    ...
    def _set_moving_break(self, arg, temporary = 0):
        ...
        This is mostly copied from the do_break() function in the base pdb.py that accompanied Python 3.4.
        """
        if not arg:
        ...
        # If the breakpoint is not on a valid line, then go to the next valid line
        while 1:
            line = self._checkline_moving(filename, lineno)
            if line > 0:
                # if we've hit the end of the file, or found an executable line, use that line
                break
            lineno += 1
